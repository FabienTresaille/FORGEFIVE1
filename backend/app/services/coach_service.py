...
def build_coach_system_prompt(user: User, recent_workouts, recent_recovery, ranks):
    return f"""You are ForgeFive Coach, a personal training assistant. 
Never give medical advice. If user mentions pain/injury, recommend seeing a healthcare professional.
User name: {user.display_name or 'User'}
Goal: {user.goal or 'General fitness'}
Recent workouts: {len(recent_workouts)} in last 14 days.
Latest recovery: {recent_recovery.sleep_hours if recent_recovery else 'N/A'} hrs sleep, energy {recent_recovery.energy_level if recent_recovery else 'N/A'}/5.
Ranks: {', '.join([f'{r.exercise_id} ({r.rank_tier})' for r in ranks]) if ranks else 'None'}
Respond in French."""
...
async def analyze_workout_session(db: AsyncSession, user: User, session_id: uuid.UUID):
    ...
    sets_info = "\n".join([f"Set {s.set_number}: {s.weight}kg x {s.reps} reps (RPE {s.rpe})" for s in session.sets])
    sys_prompt = "You are ForgeFive Coach. Analyze this workout session, give feedback, and suggest improvements in French. Never give medical advice. If user mentions pain/injury, recommend seeing a healthcare professional."
    user_msg = f"Workout info: Duration {session.duration_minutes}m. Notes: {session.notes}. Sets:\n{sets_info}"
...
async def get_daily_tip(db: AsyncSession, user: User):
    ...
    sys_prompt = "You are ForgeFive Coach. Give a single, short, personalized daily tip in French. Never give medical advice. If user mentions pain/injury, recommend seeing a healthcare professional."
    user_msg = f"Recent sleep: {recov.sleep_hours if recov else 'Unknown'}h. Recent workout duration: {work.duration_minutes if work else 'Unknown'}m."
